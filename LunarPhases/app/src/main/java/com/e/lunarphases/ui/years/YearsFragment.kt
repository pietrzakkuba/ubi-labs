...
class YearsFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        algorithm?.let { calc(year, it) }
        previousYearButton.setOnClickListener {
            algorithm?.let { it1 -> previous(it1) }
        }
        nextYearButton.setOnClickListener {
            algorithm?.let { it1 -> next(it1) }
        }
    }
    ...
    private fun calc(year: Int, alg: Int) {
        ...
        when (alg) {
            2131230788 -> {
                while (current_date != Date(year - 1900, 11, 31)) {
                    ...
                    if (Calculator().simple(local_year, local_month, local_day) == 14) {
            ...
            2131230789 -> {
                while (current_date != Date(year - 1900, 11, 31)) {
                    ...
                    local_month = calendar.get(Calendar.MONTH) + 1
                    local_day = calendar.get(Calendar.DAY_OF_MONTH)
                    if (Calculator().conway(local_year, local_month, local_day) == 15) {
                        val text = list2[i] + " " + local_year + "-" + String.format("%02d", local_month) + "-" + String.format("%02d", local_day)
                        list[i].text = text
                        i++
                    }
            ...
            2131230790 -> {
                while (current_date != Date(year - 1900, 11, 31)) {
                    ...
                    local_month = calendar.get(Calendar.MONTH) + 1
                    local_day = calendar.get(Calendar.DAY_OF_MONTH)
                    if (Calculator().trig1(local_year, local_month, local_day) == 15) {
                        val text = list2[i] + " " + local_year + "-" + String.format("%02d", local_month) + "-" + String.format("%02d", local_day)
                        list[i].text = text
                        i++
                    }
